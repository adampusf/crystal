...
module Crystal
  class Parser < Lexer
    ...
    macro parse_operator(name, next_operator, node, operators, left_associative = false)
      def parse_{{name.id}}
        location = @token.location
        ...
        while true
          ...
          when {{operators.id}}
            ...
            slash_is_regex!
            next_token_skip_space_or_newline
            right = parse_{{(left_associative ? name : next_operator).id}}
    ...
    parse_operator :pow, :prefix, "Call.new left, method, right", %(:"**", :"&**"), left_associative: true
